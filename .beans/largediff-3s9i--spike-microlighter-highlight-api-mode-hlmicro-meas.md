...
## Findings (Safari 26.6, visible window, selftest 10 jumps @1500ms gap)
...
Conclusion: the WebKit Highlight repaint cost is API-inherent, not
tokenizer-specific. Per-token spans stay the default. ?hl=micro stays
in the tree next to ?hl=ranges so the cost stays reproducible.
